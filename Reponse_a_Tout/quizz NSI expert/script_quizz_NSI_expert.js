...
async function verifier_reponse(réponse_selec) {
    ...
    if (réponse_selec == rep_corect){                                                                       // Vérifie si la réponse séléctionner est correct
        ...
        if(num_question == 1){                                                                              // Si c'est la 1er question
            question.innerText = '2. Quel est le rôle de la méthode de "backtracking" dans un algorithme ?';// Met la question de la 2
            rep1.innerHTML = "- Résoudre les problèmes de recherche en profondeur";                         // Change la reponse 1
            rep2.innerHTML = "- Résoudre des problèmes en vérifiant toutes les solutions possibles";        // Change la reponse 2
            rep3.innerHTML = "- Créer des parcours de graphes sans cycles";                                 // Change la reponse 3
            rep4.innerHTML = "- Accélérer l'exécution d'un programme récursif";                             // Change la reponse 4
            ...
            question.innerText = '3. En quoi consiste le "théorème de P vs NP" ?';                          // Met la question de la 2
            rep1.innerHTML = "- Déterminer si un problème est facile à résoudre mais difficile à vérifier"; // Change la reponse 1
            rep2.innerHTML = "- Montrer que tous les algorithmes sont équivalents";                         // Change la reponse 2
            rep3.innerHTML = "- Montrer que tous les problèmes peuvent être résolus de manière optimale";   // Change la reponse 3
            rep4.innerHTML = "- Prouver que tous les problèmes sont résolubles en temps polynomial";        // Change la reponse 4
            ...
            if(etat_connexion == "true"){                                                                   // Si l'utilisateur est connecté alors sauvegarder le calcul dans le local storage
                ...
                if(chercher_utilisateur){                                                                   // Si l'utilisateur connecté est trouvé :
                    let meilleur_score = chercher_utilisateur.stat_meilleur_score.quizz.NSI[3][1];          // Récupére l'ancien meilleur score                        
                    if(meilleur_score < score){                                                             // Si l'ancien meilleur score est inférieur au score que l'utilisateur vient de faire
                        chercher_utilisateur.stat_meilleur_score.quizz.NSI[3][1]= score;                    // Alors mettre a jour le meilleur score
                        localStorage.setItem("utilisateurs", JSON.stringify(info_joueur));                  // Enregistre les infos du joueur mis à jour
                    }                                                             
                }
            }
            alert("Félicitations ! Vous avez fini le niveau expert du test de NSI !");                      // Afficher message pour félicité
