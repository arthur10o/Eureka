...
async function verifier_reponse(réponse_selec) {
    ...
    if (réponse_selec == rep_corect){                                                                       // Vérifie si la réponse séléctionner est correct
        ...
        if(num_question == 1){                                                                              // Si c'est la 1er question
            question.innerText = '2. Dans quelle situation un objet peut-il avoir une accélération nulle, mais une vitesse non nulle ?';// Met la question de la 2
            rep1.innerHTML = "- Lorsqu'il est au repos";                                                    // Change la reponse 1
            rep2.innerHTML = "- Lorsqu'il se déplace à vitesse constante";                                  // Change la reponse 2
            rep3.innerHTML = "- Lorsqu'il est en mouvement circulaire";                                     // Change la reponse 3
            rep4.innerHTML = "- Lorsqu'il subit une force constante";                                       // Change la reponse 4
            ...
            question.innerText = "3. Laquelle des équations suivantes est correcte pour la conservation de l'énergie mécanique dans un système fermé ?";// Met la question de la 2
            rep1.innerHTML = "- W = ∆E + ∆U";                                                               // Change la reponse 1
            rep2.innerHTML = "- E_totale = E_cinétique + E_potentielle";                                    // Change la reponse 2
            rep3.innerHTML = "- E_totale = P * t";                                                          // Change la reponse 3
            rep4.innerHTML = "- E_totale = m * v";                                                          // Change la reponse 4
            rep_corect = "rep2";                                                                            // Change la reponse valide
            ...
            if(etat_connexion == "true"){                                                                   // Si l'utilisateur est connecté alors sauvegarder le calcul dans le local storage
                ...
                if(chercher_utilisateur){                                                                   // Si l'utilisateur connecté est trouvé :
                    let meilleur_score = chercher_utilisateur.stat_meilleur_score.quizz.physique[2][1];     // Récupére l'ancien meilleur score                        
                    if(meilleur_score < score){                                                             // Si l'ancien meilleur score est inférieur au score que l'utilisateur vient de faire
                        chercher_utilisateur.stat_meilleur_score.quizz.physique[2][1]= score;               // Alors mettre a jour le meilleur score
                        localStorage.setItem("utilisateurs", JSON.stringify(info_joueur));                  // Enregistre les infos du joueur mis à jour
                    }                                                             
                }
            }
            alert("Félicitations ! Vous avez fini le niveau difficile du test de physique !");              // Afficher message pour félicité
