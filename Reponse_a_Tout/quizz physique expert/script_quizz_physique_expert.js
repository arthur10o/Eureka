...
async function verifier_reponse(réponse_selec) {
    ...
    if (réponse_selec == rep_corect){                                                                       // Vérifie si la réponse séléctionner est correct
        ...
        if(num_question == 1){                                                                              // Si c'est la 1er question
            question.innerText = "2. Dans le cadre de la relativité générale, que représente la courbure de l'espace-temps ?"; // Met la question de la 2
            rep1.innerHTML = "- Les particules subatomiques interagissent";                                 // Change la reponse 1
            rep2.innerHTML = "- La gravité est perçue comme une force entre deux masses";                   // Change la reponse 2
            rep3.innerHTML = "- Les objets massifs déforment l'espace-temps autour d'eux";                  // Change la reponse 3
            rep4.innerHTML = "- La lumière se déplace plus lentement dans un champ gravitationnel";         // Change la reponse 4
            ...
            question.innerText = "3. Dans un réacteur nucléaire, quelle est la principale réaction qui libère de l'énergie ?";// Met la question de la 2
            rep1.innerHTML = "- Fission des noyaux d'uranium";                                              // Change la reponse 1
            rep2.innerHTML = "- Fusion des noyaux d'hydrogène";                                             // Change la reponse 2
            rep3.innerHTML = "- Désintégration radioactive des isotopes";                                   // Change la reponse 3
            rep4.innerHTML = "- Emission de particules alpha";                                              // Change la reponse 4
            rep_corect = "rep1";                                                                            // Change la reponse valide
            num_question = 3;                                                                               // Met à jour le numéro de la question par un 0 pour dire que c'est fini
        }
        else  if(num_question == 3){                                                                        // Si c'est la 2eme question
            question.innerText = '4. Quelle est la relation entre le facteur de Lorentz (γ) et la vitesse d’un objet selon la relativité restreinte ?';// Met la question de la 2
            rep1.innerHTML = "- γ = 1 / √(1 - v²/c²)";                                                      // Change la reponse 1
            rep2.innerHTML = "- γ = √(1 - v²/c²)";                                                          // Change la reponse 2
            rep3.innerHTML = "- γ = v²/c²";                                                                 // Change la reponse 3
            rep4.innerHTML = "- γ = 1 / (1 - v²/c²)";                                                       // Change la reponse 4
            rep_corect = "rep1";                                                                            // Change la reponse valide
            ...
            if(etat_connexion == "true"){                                                                   // Si l'utilisateur est connecté alors sauvegarder le calcul dans le local storage
                ...
                if(chercher_utilisateur){                                                                   // Si l'utilisateur connecté est trouvé :
                    let meilleur_score = chercher_utilisateur.stat_meilleur_score.quizz.physique[3][1];// Récupére l'ancien meilleur score                        
                    if(meilleur_score < score){                                                             // Si l'ancien meilleur score est inférieur au score que l'utilisateur vient de faire
                        chercher_utilisateur.stat_meilleur_score.quizz.physique[3][1]= score;                // Alors mettre a jour le meilleur score
                        localStorage.setItem("utilisateurs", JSON.stringify(info_joueur));                  // Enregistre les infos du joueur mis à jour
                    }                                                             
                }
            }
            alert("Félicitations ! Vous avez fini le niveau expert du test de physique !");                 // Afficher message pour félicité
...
let rep_corect = "rep4";                                                                                    // Définit la bonne réponse
