...
async function verifier_reponse(réponse_selec) {
    ...
    if (réponse_selec == rep_corect){                                                                       // Vérifie si la réponse séléctionner est correct
        ...
        if(num_question == 1){                                                                              // Si c'est la 1er question
            question.innerText = "2. Que représente un graphe orienté en informatique ?";                   // Met la question de la 2
            rep1.innerHTML = "- Un ensemble de points reliés par des arcs dans une direction spécifique";   // Change la reponse 1
            rep2.innerHTML = "- Un réseau de communication sans direction";                                 // Change la reponse 2
            rep3.innerHTML = "- Une série de données sous forme de table";                                  // Change la reponse 3
            rep4.innerHTML = "- Une représentation d'un programme sous forme d'arbre";                      // Change la reponse 4
            ...
            question.innerText = "3. En quoi consiste l'algorithme de tri 'quicksort' ?";                   // Met la question de la 3
            rep1.innerHTML = "- Diviser un tableau en sous-tableaux et trier chaque sous-tableau récursivement";// Change la reponse 1
            rep2.innerHTML = "- Trier les éléments de manière linéaire";                                    // Change la reponse 2
            rep3.innerHTML = "- Trier en comparant les éléments deux par deux";                             // Change la reponse 3
            rep4.innerHTML = "- Trier un tableau en l'inversant";                                           // Change la reponse 4
            rep_corect = "rep1";                                                                            // Change la reponse valide
            ...
            if(etat_connexion == "true"){                                                                   // Si l'utilisateur est connecté alors sauvegarder le calcul dans le local storage
                ...
                if(chercher_utilisateur){                                                                   // Si l'utilisateur connecté est trouvé :
                    let meilleur_score = chercher_utilisateur.stat_meilleur_score.quizz.NSI[2][1];          // Récupére l'ancien meilleur score                        
                    if(meilleur_score < score){                                                             // Si l'ancien meilleur score est inférieur au score que l'utilisateur vient de faire
                        chercher_utilisateur.stat_meilleur_score.quizz.NSI[2][1]= score;                    // Alors mettre a jour le meilleur score
                        localStorage.setItem("utilisateurs", JSON.stringify(info_joueur));                  // Enregistre les infos du joueur mis à jour
                    }                                                             
                }
            }
            alert("Félicitations ! Vous avez fini le niveau difficile du test de NSI !");                   // Afficher message pour félicité
...
let rep_corect = "rep2";                                                                                    // Définit la bonne réponse
