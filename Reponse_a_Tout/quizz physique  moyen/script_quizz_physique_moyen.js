...
async function verifier_reponse(réponse_selec) {
    ...
    if (réponse_selec == rep_corect){                                                                       // Vérifie si la réponse séléctionner est correct
        ...
        if(num_question == 1){                                                                              // Si c'est la 1er question
            question.innerText = "2. Quel est le rôle d'un condensateur dans un circuit électrique ?";      // Met la question de la 2
            rep1.innerHTML = "- Stocker l'énergie électromagnétique";                                       // Change la reponse 1
            rep2.innerHTML = "- Limiter la tension";                                                        // Change la reponse 2
            rep3.innerHTML = "- Stocker l'énergie électrique";                                              // Change la reponse 3
            rep4.innerHTML = "- Générer un courant continu";                                                // Change la reponse 4
            rep_corect = "rep3";                                                                            // Change la reponse valide
            num_question = 2;                                                                               // Met à jour le numéro de la question
        }
        else  if(num_question == 2){                                                                        // Si c'est la 1er question
            question.innerText = '3. Quel phénomène est responsable des aurores boréales ?';                // Met la question de la 2
            rep1.innerHTML = "- La réflexion de la lumière sur les nuages";                                 // Change la reponse 1
            rep2.innerHTML = "- La décharge électrique dans les orages";                                    // Change la reponse 2
            rep3.innerHTML = "- Les particules solaires entrant en collision avec l'atmosphère terrestre";  // Change la reponse 3
            rep4.innerHTML = "- Les rayons cosmiques frappant la Terre";                                    // Change la reponse 4
            rep_corect = "rep3";                                                                            // Change la reponse valide
            ...
            if(etat_connexion == "true"){                                                                   // Si l'utilisateur est connecté alors sauvegarder le calcul dans le local storage
                ...
                if(chercher_utilisateur){                                                                   // Si l'utilisateur connecté est trouvé :
                    let meilleur_score = chercher_utilisateur.stat_meilleur_score.quizz.physique[1][1];     // Récupére l'ancien meilleur score                        
                    if(meilleur_score < score){                                                             // Si l'ancien meilleur score est inférieur au score que l'utilisateur vient de faire
                        chercher_utilisateur.stat_meilleur_score.quizz.physique[1][1]= score;               // Alors mettre a jour le meilleur score
                        localStorage.setItem("utilisateurs", JSON.stringify(info_joueur));                  // Enregistre les infos du joueur mis à jour
                    }                                                             
                }
            }
            alert("Félicitations ! Vous avez fini le niveau moyen du test de physique !");                  // Afficher message pour félicité
