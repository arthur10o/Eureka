...
async function verifier_reponse(réponse_selec) {
    ...
    if (réponse_selec == rep_corect){                                                                       // Vérifie si la réponse séléctionner est correct
        ...
        if(num_question == 1){                                                                              // Si c'est la 1er question
            question.innerText = "2. Dans quel pays se trouve la célèbre statue du Christ Rédempteur ?";    // Met la question de la 2
            rep1.innerHTML = "- Espagne";                                                                   // Change la reponse 1
            rep2.innerHTML = "- Argentine";                                                                 // Change la reponse 2
            rep3.innerHTML = "- Brésil";                                                                    // Change la reponse 3
            ...
            question.innerText = '3. Quel est le plus grand mammifère du monde ?';                          // Met la question de la 2
            rep1.innerHTML = "- L'éléphant d’Afrique";                                                      // Change la reponse 1
            rep2.innerHTML = "- Le cachalot";                                                               // Change la reponse 2
            rep3.innerHTML = "- La baleine bleue";                                                          // Change la reponse 3
            rep4.innerHTML = "- L’hippopotame";                                                             // Change la reponse 4
            rep_corect = "rep3";                                                                            // Change la reponse valide
            ...
            if(etat_connexion == "true"){                                                                   // Si l'utilisateur est connecté alors sauvegarder le calcul dans le local storage
                ...
                if(chercher_utilisateur){                                                                   // Si l'utilisateur connecté est trouvé :
                    let meilleur_score = chercher_utilisateur.stat_meilleur_score.quizz.culture_G[1][1];// Récupére l'ancien meilleur score                        
                    if(meilleur_score < score){                                                             // Si l'ancien meilleur score est inférieur au score que l'utilisateur vient de faire
                        chercher_utilisateur.stat_meilleur_score.quizz.culture_G[1][1]= score;                // Alors mettre a jour le meilleur score
                        localStorage.setItem("utilisateurs", JSON.stringify(info_joueur));                  // Enregistre les infos du joueur mis à jour
                    }                                                             
                }
            }
            alert("Félicitations ! Vous avez fini le niveau moyen du test de culture G !");                 // Afficher message pour félicité
