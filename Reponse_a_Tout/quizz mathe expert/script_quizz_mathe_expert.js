...
async function verifier_reponse(réponse_selec) {
    ...
    if (réponse_selec == rep_corect){                                                                       // Vérifie si la réponse séléctionner est correct
        ...
        if(num_question == 1){                                                                              // Si c'est la 1er question
            question.innerText = "2. Résoudre l'intégrale : ∫(3x²+2x)dx. Quel est le résultat ?";           // Met la question de la 2
            rep1.innerHTML = "- x³ + x² + C";                                                               // Change la reponse 1
            rep2.innerHTML = "- x³ + 2x + C";                                                               // Change la reponse 2
            rep3.innerHTML = "- x² + x³ + C";                                                               // Change la reponse 3
            rep4.innerHTML = "- 3x³ + 2x² + C";                                                             // Change la reponse 4
            ...
            question.innerText = '3. Dans un triangle équilatéral, si un côté mesure 8 cm, quelle est sa hauteur ?';// Met la question de la 2
            rep1.innerHTML = "- 6,93 cm";                                                                   // Change la reponse 1
            rep2.innerHTML = "- 4 cm";                                                                      // Change la reponse 2
            rep3.innerHTML = "- 5 cm";                                                                      // Change la reponse 3
            rep4.innerHTML = "- 7 cm";                                                                      // Change la reponse 4
            rep_corect = "rep1";                                                                            // Change la reponse valide
            ...
            if(etat_connexion == "true"){                                                                   // Si l'utilisateur est connecté alors sauvegarder le calcul dans le local storage
                ...
                if(chercher_utilisateur){                                                                   // Si l'utilisateur connecté est trouvé :
                    let meilleur_score = chercher_utilisateur.stat_meilleur_score.quizz.mathe[3][1];        // Récupére l'ancien meilleur score                        
                    if(meilleur_score < score){                                                             // Si l'ancien meilleur score est inférieur au score que l'utilisateur vient de faire
                        chercher_utilisateur.stat_meilleur_score.quizz.mathe[3][1]= score;                  // Alors mettre a jour le meilleur score
                        localStorage.setItem("utilisateurs", JSON.stringify(info_joueur));                  // Enregistre les infos du joueur mis à jour
                    }                                                             
                }
            }
            alert("Félicitations ! Vous avez fini le niveau expert du test de mathe !");                    // Afficher message pour félicité
