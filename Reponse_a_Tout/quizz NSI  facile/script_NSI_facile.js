...
async function verifier_reponse(réponse_selec) {
    ...
    if (réponse_selec == rep_corect){                                                                       // Vérifie si la réponse séléctionner est correct
        ...
        if(num_question == 1){                                                                              // Si c'est la 1er question
            question.innerText = "2. Quel est le rôle d’un processeur dans un ordinateur ?";                // Met la question de la 2
            rep1.innerHTML = "- Stocker les données";                                                       // Change la reponse 1
            rep2.innerHTML = "- Exécuter des instructions";                                                 // Change la reponse 2
            rep3.innerHTML = "- Afficher l’image à l’écran";                                                // Change la reponse 3
            rep4.innerHTML = "- Envoyer des données par internet";                                          // Change la reponse 4
            ...
            question.innerText = "3. Quel type de données est représenté par le nombre 5 ?";                // Met la question de la 2
            rep1.innerHTML = "- Booléen";                                                                   // Change la reponse 1
            rep2.innerHTML = "- Entier";                                                                    // Change la reponse 2
            rep3.innerHTML = "- Chaine de caractères";                                                      // Change la reponse 3
            rep4.innerHTML = "- Flottant";                                                                  // Change la reponse 4
            rep_corect = "rep2";                                                                            // Change la reponse valide
            ...
            if(etat_connexion == "true"){                                                                   // Si l'utilisateur est connecté alors sauvegarder le calcul dans le local storage
                ...
                if(chercher_utilisateur){                                                                   // Si l'utilisateur connecté est trouvé :
                    let meilleur_score = chercher_utilisateur.stat_meilleur_score.quizz.NSI[0][1];          // Récupére l'ancien meilleur score                        
                    if(meilleur_score < score){                                                             // Si l'ancien meilleur score est inférieur au score que l'utilisateur vient de faire
                        chercher_utilisateur.stat_meilleur_score.quizz.NSI[0][1]= score;                    // Alors mettre a jour le meilleur score
                        localStorage.setItem("utilisateurs", JSON.stringify(info_joueur));                  // Enregistre les infos du joueur mis à jour
                    }                                                             
                }
            }
            alert("Félicitations ! Vous avez fini le niveau facile du test de NSI !");                      // Afficher message pour félicité
